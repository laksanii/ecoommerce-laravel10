    ...
    <div class="card">
        ...
        <div class="card-body">
            @if ($order)
                ...
                <!-- Ordered Items (prefer order_items table, fallback to cart_info) -->
                @php
                    $items =
                        $order->order_items && count($order->order_items) ? $order->order_items : $order->cart_info;
                    $i = 1;
                @endphp
                @if ($items && count($items) > 0)
                    <section class="order_items section_padding">
                                ...
                                <tbody>
                                    @foreach ($items as $it)
                                        <tr>
                                            <td>{{ $i++ }}</td>
                                            <td>
                                                {{-- If this item is an OrderItem it won't have relation 'product'; if it's a Cart it may --}}
                                                @if (isset($it->product) && $it->product)
                                                    <a
                                                        href="{{ route('product-detail', $it->product->slug) }}">{{ $it->product->title }}</a>
                                                @elseif(!empty($it->product_title))
                                                    {{ $it->product_title }}
                                                @else
